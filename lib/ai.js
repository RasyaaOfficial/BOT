...
if (rule.action && rule.action.features === 'generate_image_imagen') {
    try {
        ...
        // 1. Inisialisasi model dengan API Key dari config
        const genAI = new GoogleGenerativeAI(config.GEMINI_API_KEY);
        const model = genAI.getGenerativeModel({ model: "gemini-1.5-flash-latest" });

        const prompt = rule.action.content;
        
        console.log(`[IMAGEN] Memulai generasi gambar untuk prompt: "${prompt}"`);

        // 2. Panggil API untuk men-generate gambar
        const result = await model.generateContent([
          "Generate a high-quality, photorealistic image based on the following prompt.", // Instruksi tambahan untuk hasil lebih baik
          {
            "image_generation": {
              "prompt": prompt,
              "number_of_images": 2, // Kita buat 2 gambar agar tidak terlalu banyak spam
            }
          }
        ]);

        const response = await result.response;
        const generatedImages = response.candidates[0].content.parts.filter(part => part.image_generation);
        
        if (!generatedImages || generatedImages.length === 0) {
            throw new Error('Model AI tidak menghasilkan gambar apa pun.');
        }

        console.log(`[IMAGEN] Berhasil mendapatkan ${generatedImages.length} gambar.`);
        ...
        for (const generatedImage of generatedImages) {
            ...
            const base64Image = generatedImage.image_generation.image.b64_encoded_image;
        ...
        console.error("🚫 Imagen Gagal:", error);
        await sock.sendMessage(remoteJid, { text: `Maaf, terjadi kesalahan saat membuat gambar.\n\n*Pesan Error:* ${error.message}` }, { quoted: message });
    }
}
