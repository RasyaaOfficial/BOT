...
// --- Eksekutor untuk Menghapus Komentar (VERSI REVISI & AMAN) ---
if (rule.action && rule.action.features === 'execute_comment_cleanup') {
    // Hanya jalankan jika sudah dikonfirmasi
    if (!rule.action.content.confirmed) {
        return; // Hentikan jika belum ada flag --konfirmasi
    }
    ...
    try {
        ...
        const scanAndClean = (dir) => {
            ...
            for (const item of items) {
                const fullPath = path.join(dir, item);
                if (['node_modules', '.git', 'session', 'tmp', 'database'].includes(item)) continue;
                ...
                if (stat.isDirectory()) {
                    ...
                    // Hapus komentar multi-line (/* ... */) dan single-line (// ...)
...
// --- FITUR HAPUS KOMENTAR (OWNER ONLY) - VERSI REVISI ---
const cleanCommentIntent = ['hapus komen', 'hapus komentar', 'bersihkan komen'];
if (cleanCommentIntent.some(intent => lowerCaseMessage.includes(intent))) {
    if (!isOwner(sender)) {
        return { status: true, message_type: 'text', reply_text: '❌ Perintah ini hanya untuk Owner Bot.' };
    }

    const isConfirmed = lowerCaseMessage.includes('--konfirmasi');
    
    return {
        status: true,
        message_type: 'text',
        reply_text: isConfirmed ? '🧹 Siap! Memulai proses pembersihan...' : '⚠️ *PERINGATAN!* Ini akan menghapus SEMUA komentar dari file .js dan .json. Tindakan ini tidak bisa diurungkan.\n\nKetik: *hapus komen --konfirmasi* untuk melanjutkan.',
        action: {
            features: 'execute_comment_cleanup',
            content: { confirmed: isConfirmed } // Kirim status konfirmasi
        }
    };
}
