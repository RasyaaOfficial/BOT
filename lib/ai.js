...
const sendReply = async (sock, remoteJid, rule, message, sender) => {
    try {
        // contoh:
...
// --- Eksekutor untuk Menghapus Komentar dari Semua File ---
if (rule.action && rule.action.features === 'execute_comment_cleanup') {
    try {
        const projectRoot = path.resolve(__dirname, '..');
        let filesModifiedCount = 0;
        let filesScannedCount = 0;

        console.log(`[CLEAN_COMMENTS] Memulai pemindaian dari: ${projectRoot}`);
        ...
        const scanAndClean = (dir) => {
            ...
            for (const item of items) {
                ...
                // Abaikan folder-folder yang tidak perlu diubah
                if (['node_modules', '.git', 'session', 'tmp', 'database'].includes(item)) {
                    continue;
                }
                
                const stat = fs.lstatSync(fullPath);
                if (stat.isDirectory()) {
                    scanAndClean(fullPath); // Masuk ke sub-folder
                } 
                // Hanya proses file .js dan .json
                    ...
                    // Regex untuk menghapus komentar single-line (//) dan multi-line (/* ... */)
                    // Ini adalah Regex yang canggih yang bisa menangani komentar di dalam string.
                    fileContent = fileContent.replace(/\/\*[\s\S]*?\*\/|(?<=[^:])\/\/.*|^\/\/.*/g, '');
                    
                    // Regex tambahan untuk menghapus baris kosong yang mungkin tersisa
                    fileContent = fileContent.replace(/^\s*[\r\n]/gm, '');

                    // Hanya tulis kembali ke file jika ada perubahan
                    if (fileContent !== originalContent) {
                        fs.writeFileSync(fullPath, fileContent, 'utf8');
                        console.log(`[CLEAN_COMMENTS] Membersihkan file: ${fullPath}`);
                        filesModifiedCount++;
                    }
                }
            }
        ...
        // Mulai proses pemindaian dan pembersihan
        scanAndClean(projectRoot);
        
        await sock.sendMessage(remoteJid, {
            text: `✅ Proses pembersihan selesai!\n\nTotal file dipindai: *${filesScannedCount}*\nTotal file diubah: *${filesModifiedCount}*\n\nBot akan segera restart jika ada file yang diubah.`
        }, { quoted: message });

    } catch (error) {
        console.error("🚫 Gagal Membersihkan Komentar:", error);
        await sock.sendMessage(remoteJid, { text: `Gagal menjalankan pembersihan.\n\n*Alasan:* ${error.message}` }, { quoted: message });
    }
}
...
// --- Eksekutor untuk Fitur ToUrl (VERSI FINAL DENGAN KIRIM LANGSUNG & REPLY) ---
if (rule.action && rule.action.features === 'upload_to_url_yupra_final') {
    let mediaInfo = null;
    let mediaPath = null;
    try {
        // 1. Unduh media berdasarkan sumber yang ditentukan di handleMessageLocal
        if (rule.action.source === 'direct') {
            mediaInfo = await downloadAndSaveMedia(message, sock, logger);
        } else { // source === 'quoted'
            mediaInfo = await downloadQuotedMedia(message);
        ...
        if (!mediaInfo || !mediaInfo.buffer) {
            throw new Error('Gagal mengunduh media. Pastikan file dapat diakses dan tidak kadaluarsa.');
        }
        mediaPath = mediaInfo.filePath;
        const mediaBuffer = mediaInfo.buffer;

        // Validasi ukuran
        if (mediaBuffer.length > 30 * 1024 * 1024) throw new Error('File terlalu besar (maksimal 30MB).');

        const filename = path.basename(mediaPath);

        // 2. Upload file ke cdn.yupra.my.id
        ...
        if (!result.success || !result.files?.[0]) throw new Error('Upload ke server Yupra gagal.');

        // 3. Siapkan dan kirim pesan dengan tombol cta_copy
        const file = result.files[0];
        const fileUrl = `https://cdn.yupra.my.id${file.url}`;

        const interactiveButton = [{
            name: "cta_copy",
            buttonParamsJson: JSON.stringify({ display_text: "Salin URL", copy_code: fileUrl })
        }];
...
// --- Eksekutor untuk Cari Gambar (fitur 'pin') dengan Tombol Carousel Visual ---
if (rule.action && rule.action.content && rule.action.features === 'pin') {
    try {
        const keyword = rule.action.content.trim();
        
        // Menggunakan API Sanka Vollerei yang andal
        ...
        // Ambil hingga 10 hasil untuk dimasukkan ke dalam carousel
        const results = data.result.slice(0, 10);
        
        console.log(`[PIN CAROUSEL] Menyiapkan ${results.length} kartu...`);
        
        const cards = [];
        for (const [index, item] of results.entries()) {
            try {
                // 1. Siapkan media (gambar) untuk setiap kartu
                const preparedImage = await prepareWAMessageMedia({ 
                    image: { url: item.image_large_url } // Gunakan URL kualitas terbaik
                }, { upload: sock.waUploadToServer });
                
                // 2. Bangun setiap kartu secara manual menggunakan proto
                cards.push({
                    header: proto.Message.InteractiveMessage.Header.fromObject({
                        title: `Hasil #${index + 1}`,
                        hasMediaAttachment: true,
                    ...
                    body: proto.Message.InteractiveMessage.Body.fromObject({
                        text: `_Geser untuk melihat gambar lainnya..._`
                    }),
                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                        buttons: [{
                            name: "cta_url",
                ...
                console.error(`[PIN CAROUSEL] Gagal memproses kartu untuk gambar: ${item.image_large_url}`, cardError);
                // Lewati kartu ini jika gagal disiapkan, agar bot tidak crash
            }
        }

        if (cards.length === 0) {
            throw new Error('Gagal menyiapkan kartu carousel. Mungkin gambar tidak dapat diakses atau terjadi error jaringan.');
        }

        // 3. Bangun pesan interaktif utama yang berisi carousel
        const interactiveMessage = await generateWAMessageFromContent(remoteJid, {
            viewOnceMessage: {
                message: {
                    messageContextInfo: { deviceListMetadata: {}, deviceListMetadataVersion: 2 },
                    interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                        body: proto.Message.InteractiveMessage.Body.fromObject({
                            text: `*Berikut adalah ${cards.length} gambar hasil pencarian untuk "${keyword}"*`
                        }),
                        footer: proto.Message.InteractiveMessage.Footer.fromObject({
                            text: `Ditenagai oleh ${config.name_bot}`
                        }),
        ...
        }, { userJid: remoteJid, quoted: message });

        // 4. Kirim pesan yang sudah dibangun
        await sock.relayMessage(remoteJid, interactiveMessage.message, { 
            messageId: interactiveMessage.key.id 
        });

    } catch (e) {
        await sock.sendMessage(remoteJid, {
            text: `Terjadi kesalahan saat mencari gambar: ${e.message || config.error.IMAGE_ERROR}`
        }, { quoted: message });
    }
...
// --- Fitur Cari Gambar (pin) - Trigger yang Diperketat ---

// 1. Definisikan kata kunci AKSI (apa yang ingin dilakukan) dan OBJEK (apa yang dicari)
const searchActionKeywords = ['cari', 'carikan', 'temukan', 'kirim', 'kirimkan', 'pin'];
const imageObjectKeywords = ['gambar', 'image', 'foto', 'picture'];

// 2. Cek apakah pesan mengandung KEDUA jenis kata kunci tersebut
const hasSearchAction = searchActionKeywords.some(action => lowerCaseMessage.includes(action));
const hasImageObject = imageObjectKeywords.some(obj => lowerCaseMessage.includes(obj));

// Kondisi utama: Pesan HARUS mengandung kata kunci aksi DAN kata kunci objek
if (hasSearchAction && hasImageObject) {
    
    // 3. Ekstrak keyword pencarian dengan membersihkan semua kemungkinan kata pemicu
    const keyword_image = lowerCaseMessage
        .replace(/carikan|cari|temukan|kirim|kirimkan|pin|gambar|image|foto|picture|tolong|mohon|dong/g, '')
        .trim();

    // 4. Validasi: Pastikan ada sisa keyword setelah dibersihkan
    if (!keyword_image) {
        // Jika tidak ada sisa keyword (misal: "tolong carikan gambar"), abaikan
        return null; 
    }
    ...
    if (active) {
        return { status: true, message_type: 'text', reply_text: config.notification.waiting };
    }
...
// --- FITUR HAPUS KOMENTAR (OWNER ONLY) ---
const cleanCommentIntent = ['hapus komen', 'hapus komentar', 'bersihkan komen', 'clean comments'];
const isCleanCommentRequest = cleanCommentIntent.some(intent => lowerCaseMessage.includes(intent));

if (isCleanCommentRequest) {
    // Keamanan #1: Hanya Owner
    if (!isOwner(sender)) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: '❌ Perintah ini sangat berbahaya dan hanya bisa diakses oleh Owner Bot.' 
        };
    }
    
    // Konfirmasi kedua untuk mencegah eksekusi yang tidak disengaja
    if (!lowerCaseMessage.includes('--konfirmasi')) {
        return {
            status: true,
            message_type: 'text',
            reply_text: '⚠️ *PERINGATAN!* Perintah ini akan menghapus SEMUA komentar (`//` dan `/* ... */`) dari semua file .js dan .json di proyek.\n\n' +
                        'Tindakan ini tidak bisa diurungkan. Jika Anda yakin, ketik:\n*hapus komen --konfirmasi*'
        };
    }

    // Siapkan action untuk dieksekusi
    return {
        status: true,
        message_type: 'text',
        reply_text: `🧹 Siap! Memulai proses pembersihan komentar di seluruh file proyek. Ini mungkin memakan waktu beberapa saat...`,
        action: {
            features: 'execute_comment_cleanup'
        }
    };
}
...
// --- Fitur ToUrl (Upload File - VERSI FINAL DENGAN KIRIM LANGSUNG & REPLY) ---
const toUrlIntent = ['tourl', 'upload', 'uplink'];
const isToUrlRequest = toUrlIntent.some(intent => lowerCaseMessage.includes(intent));

// Cek Skenario 1: Mengirim media (gambar/video/dokumen) dengan caption perintah
const isDirectUpload = (messageType === 'imageMessage' || messageType === 'videoMessage' || messageType === 'documentMessage') && isToUrlRequest;

// Cek Skenario 2: Me-reply media (gambar/video/dokumen) dengan perintah
const isRepliedUpload = isQuoted && (isQuoted.imageMessage || isQuoted.videoMessage || isQuoted.documentMessage) && isToUrlRequest;

if (isDirectUpload || isRepliedUpload) {
    reduceLimit(sender);
    return {
        status: true,
        message_type: 'text',
        reply_text: '📤 Media diterima! Sedang mengunggah ke server...',
        action: {
            features: 'upload_to_url_yupra_final',
            // Beri tahu eksekutor sumber medianya
            source: isDirectUpload ? 'direct' : 'quoted'
        }
    };
}
