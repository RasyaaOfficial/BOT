...
if (rule.action && rule.action.features === 'ptero_create_server_v2') {
    try {
        ...
        const nestId = 6; // Nest ID untuk Docker (biasanya 6), sesuaikan jika perlu
        const eggId = config.PTERO_EGG_ID;
        const locId = config.PTERO_LOCATION_ID;

        // 1. Definisikan alokasi resource menggunakan resourceMap
        const resourceMap = {
        ...
        if (!resources) {
            throw new Error(`Spesifikasi RAM "${ram}GB" tidak valid. Pilihan: 1-5, unli.`);
        }
        
        // 2. Validasi nomor target
        const [onWa] = await sock.onWhatsApp(targetJid);
        if (!onWa?.exists) throw new Error(`Nomor target tidak terdaftar di WhatsApp.`);

        // 3. Siapkan data untuk membuat user
        const email = `${username}@${new URL(domain).hostname}`;
        const name = username.charAt(0).toUpperCase() + username.slice(1);
        const password = `${username}Bot${Math.floor(Math.random() * 1000)}`;

        console.log(`[PTERO] Membuat user: ${username}`);
        ...
        if (userRes.status === 422) throw new Error(`Username atau email "${username}" sudah digunakan.`);
        if (userRes.status !== 201) throw new Error(`Gagal membuat user (Status: ${userRes.status})`);
        const userData = userRes.data.attributes;

        // 4. Ambil data Egg untuk startup command
        const eggRes = await axios.get(`${domain}/api/application/nests/${nestId}/eggs/${eggId}`, {
             headers: { 'Authorization': `Bearer ${apikey}` }
        });
        const eggData = eggRes.data.attributes;

        // 5. Buat server
        console.log(`[PTERO] Membuat server untuk user ID: ${userData.id}`);
        const serverRes = await axios.post(`${domain}/api/application/servers`, {
            name: `🟢 ${name}'s Server`,
            description: `Dibuat oleh ${pushName || 'Bot'}`,
            user: userData.id,
            egg: parseInt(eggId),
            docker_image: "ghcr.io/parkervcp/yolks:nodejs_20", // Sesuaikan jika perlu
            startup: eggData.startup,
            environment: { INST: "npm", USER_UPLOAD: "0", AUTO_UPDATE: "0", CMD_RUN: "npm start" },
            limits: { memory: parseInt(resources.memory), swap: 0, disk: parseInt(resources.disk), io: 500, cpu: parseInt(resources.cpu) },
            feature_limits: { databases: 5, backups: 5, allocations: 5 },
            deploy: { locations: [parseInt(locId)], dedicated_ip: false, port_range: [] }
        }, { headers: { 'Authorization': `Bearer ${apikey}`, 'Content-Type': 'application/json', 'Accept': 'application/json' } });

        if (serverRes.status === 422) throw new Error('Gagal deploy server: kemungkinan node penuh atau error konfigurasi.');
        if (serverRes.status !== 201) throw new Error(`Gagal membuat server (Status: ${serverRes.status})`);
        const serverData = serverRes.data.attributes;

        // 6. Siapkan dan kirim pesan interaktif
        const caption = `*NEO PANEL ACCESS*\n\n` +
                        `*Username:* ${username}\n` +
                        `*Password:* ${password}\n` +
                        `*Server ID:* ${serverData.id}\n\n` +
                        `*Spesifikasi:* ${ram === "unli" ? "Unlimited" : ram.toUpperCase() + "B"} RAM\n` +
                        `*Login:* ${domain}\n\n` +
                        `╭─❖「 RULES 」❖─\n` +
                        `│ 1. Garansi hanya berlaku 15 hari.\n` +
                        `│ 2. Dilarang menggunakan untuk hal ilegal.\n` +
                        `│ 3. Jika abuse = Suspend permanen.\n` +
                        `╰──⭓ Terima Kasih!`;
        ...
        await sock.sendMessage(remoteJid, {
            text: `[ ✓ ] Server *${username}* berhasil dibuat & dikirim ke @${targetJid.split("@")[0]}.`,
            mentions: [targetJid]
        }, { quoted: message });
...
if (lowerCaseMessage.startsWith('.copy')) {
    const textToCopy = content.substring(6).trim();
    // Kirim kembali teksnya agar mudah disalin oleh pengguna
    return { 
        status: true, 
        message_type: 'text', 
...
if (lowerCaseMessage.startsWith('.openlink')) {
    const urlToOpen = content.substring(10).trim();
    // Kirim URL sebagai teks
    return { 
        status: true, 
        message_type: 'text', 
        reply_text: `Silakan buka tautan ini di browser Anda:\n${urlToOpen}` 
    };
}
...
if (rule.action && rule.action.features === 'upload_to_url_yupra') {
    let mediaInfo = null;
    let mediaPath = null;
    try {
        // Unduh media yang di-reply
        mediaInfo = await downloadQuotedMedia(message);
        if (!mediaInfo || !mediaInfo.buffer) throw new Error('Gagal mengunduh file yang di-reply.');
        mediaPath = mediaInfo.filePath;

        const mediaBuffer = mediaInfo.buffer;
        if (mediaBuffer.length > 100 * 1024 * 1024) throw new Error('File terlalu besar (maks 100MB).');

        // Deteksi tipe file untuk nama yang benar
        const { fileTypeFromBuffer } = await (await import('file-type')).default;
        const type = await fileTypeFromBuffer(mediaBuffer);
        const filename = `file_${Date.now()}${type ? '.' + type.ext : ''}`;
        ...
        const fileSize = (file.sizeInBytes / 1024).toFixed(2) + ' KB'; // Konversi ke KB
...
if (rule.action && rule.action.content && rule.action.features === 'pin') {
    try {
        ...
        // Menggunakan API dari elrayyxml yang sudah kita bahas
        const apiUrl = `https://elrayyxml.vercel.app/search/pinterest?q=${encodeURIComponent(keyword)}`;
        const { data } = await axios.get(apiUrl);
        ...
        for (const item of results) {
            try {
                ...
                // 2. Bangun setiap kartu secara manual
                cards.push({
                    header: proto.Message.InteractiveMessage.Header.fromObject({
                        title: item.grid_title || keyword.substring(0, 20),
                        hasMediaAttachment: true,
                        ...preparedImage
                    }),
                    body: proto.Message.InteractiveMessage.Body.fromObject({
                    ...
                    nativeFlowMessage: proto.Message.InteractiveMessage.NativeFlowMessage.fromObject({
                        buttons: [{
                            name: "cta_url",
                            buttonParamsJson: JSON.stringify({
                                display_text: "Buka Tautan Gambar",
                                url: item.images_url,
                                merchant_url: item.pin // Link ke halaman pinterest-nya
                            })
                        }]
                    })
                });
            } catch (cardError) {
                console.error(`[PIN CAROUSEL] Gagal memproses kartu untuk gambar: ${item.images_url}`, cardError);
                // Lewati kartu ini jika gagal disiapkan
            }
        }
        ...
        const interactiveMessage = await generateWAMessageFromContent(remoteJid, {
            viewOnceMessage: { // Pesan carousel biasanya lebih baik sebagai viewOnce
                message: {
                    messageContextInfo: {
                        deviceListMetadata: {},
                    ...
                    interactiveMessage: proto.Message.InteractiveMessage.fromObject({
                        ...
                        footer: proto.Message.InteractiveMessage.Footer.fromObject({
                            text: `Dibuat oleh ${config.name_bot}`
                        }),
...
const toUrlTriggers = ['tourl', 'upload', 'uplink', 'link'];
const containsToUrlTrigger = toUrlTriggers.some(word => lowerCaseMessage.includes(word));

// Fitur ini HANYA bekerja dengan me-reply sebuah media
if (isQuoted && containsToUrlTrigger) {
    const quotedMessage = message.message.extendedTextMessage.contextInfo.quotedMessage;
    // Pastikan yang di-reply bukan teks biasa
    if (quotedMessage.conversation) {
        return { status: true, message_type: 'text', reply_text: 'Perintah ini harus digunakan dengan me-reply sebuah file (gambar, video, dokumen, dll).' };
    }
    ...
    return {
        ...
        action: {
            features: 'upload_to_url_yupra'
            // source tidak perlu, karena kita hanya akan mendownload dari reply
        }
    };
}
