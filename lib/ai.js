...
// --- Eksekutor untuk Fitur ToUrl (dengan Tombol Copy) ---
if (rule.action && rule.action.features === 'upload_to_url_yupra_v2') {
    let mediaInfo = null;
    let mediaPath = null;
    try {
        // 1. Unduh media yang di-reply
        mediaInfo = await downloadQuotedMedia(message);
        if (!mediaInfo || !mediaInfo.buffer) {
            throw new Error('Gagal mengunduh file yang di-reply.');
        }
        mediaPath = mediaInfo.filePath;
        ...
        // 2. Deteksi tipe file untuk nama yang benar (diadaptasi dari plugin)
        const { fileTypeFromBuffer } = await (eval('import("file-type")'));
        const type = await fileTypeFromBuffer(mediaBuffer);
        let ext = type?.ext;
        
        // Fallback jika file-type gagal
        if (!ext) {
            const quotedMsg = message.message.extendedTextMessage.contextInfo.quotedMessage;
            const mime = quotedMsg.imageMessage?.mimetype || quotedMsg.videoMessage?.mimetype || quotedMsg.documentMessage?.mimetype || '';
            if (mime) ext = mime.split('/')[1]?.split(';')[0];
        }
...
// Fitur ini hanya bekerja saat me-reply sebuah media
if (isQuoted && isToUrlRequest) {
    ...
    return {
        ...
        action: {
            features: 'upload_to_url_yupra_v2' // Nama fitur baru yang canggih
        }
    };
}
