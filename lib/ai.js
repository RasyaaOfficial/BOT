...
if (rule.action && rule.action.features === 'replace_file_from_document') {
    let tempFilePath = null;
    try {
        const { targetPath } = rule.action.content;

        // 1. Unduh file dokumen yang di-reply
        const media = await downloadQuotedMedia(message);
        if (!media || !media.filePath) {
            throw new Error('Gagal mengunduh file dokumen yang di-reply.');
        }
        tempFilePath = media.filePath;

        // 2. Baca konten kode baru dari file yang diunduh
        const newCode = fs.readFileSync(tempFilePath, 'utf8');

        // 3. Bangun path absolut yang aman dari root proyek
        const fullPath = path.resolve(__dirname, '..', targetPath);
        const projectRoot = path.resolve(__dirname, '..');

        // Pengecekan keamanan terakhir: pastikan tidak menulis di luar folder proyek
        if (!fullPath.startsWith(projectRoot)) {
            throw new Error('Upaya penulisan file di luar direktori proyek terdeteksi dan diblokir.');
        }

        // 4. Tulis (timpa) file target dengan kode baru
        fs.writeFileSync(fullPath, newCode);

        // 5. Kirim pesan sukses
        await sock.sendMessage(remoteJid, {
            text: `✅ Berhasil! File *${targetPath}* telah diperbarui.\n\nLauncher akan otomatis me-restart bot untuk menerapkan perubahan.`
        }, { quoted: message });

    } catch (error) {
        console.error("🚫 Ganti File (dari dokumen) Gagal:", error);
        await sock.sendMessage(remoteJid, { text: `Gagal memperbarui file.\n\n*Alasan:* ${error.message}` }, { quoted: message });
    } finally {
        // 6. Selalu hapus file .js sementara yang diunduh
...
if (rule.action && rule.action.features === 'generate_image_imagen') {
    try {
        ...
        // 2. Panggil metode 'generateImages()' yang benar sesuai dokumentasi
...
if (gantiFileTriggerWord) {
    // ===================================================================
    // PENGECEKAN KEAMANAN #1: HANYA OWNER
    // ===================================================================
    if (!isOwner(sender)) {
        return { 
            status: true, 
    ...
    if (!isQuoted) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: 'Untuk menggunakan fitur ini, Anda harus me-reply file dokumen .js yang berisi kode baru.' 
        };
    }
    ...
    if (filePathToReplace.includes('..') || !filePathToReplace.endsWith('.js')) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: '❌ Path tidak valid atau bukan file .js. Anda hanya bisa mengganti file JavaScript.' 
        };
    }
    ...
    // Validasi #2: Pastikan yang di-reply adalah dokumen dan berakhiran .js
    if (!quotedMessage.documentMessage || !quotedMessage.documentMessage.fileName.endsWith('.js')) {
        return {
            status: true,
            message_type: 'text',
            reply_text: '❌ Anda harus me-reply sebuah file dokumen yang berakhiran .js.'
        };
    }
    return {
        status: true,
