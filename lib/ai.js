...
// --- Eksekutor untuk Fitur Text-to-Image (Imagen 4 via SDK Terbaru) ---
if (rule.action && rule.action.features === 'generate_image_imagen') {
    try {
        // 1. Impor dan inisialisasi SDK Google GenAI
        const { GoogleGenerativeAI } = require("@google/generative-ai");
        const genAI = new GoogleGenerativeAI(config.GEMINI_API_KEY);
        
        const prompt = rule.action.content;

        console.log(`[IMAGEN 4] Memulai generasi gambar dengan model terbaru untuk: "${prompt}"`);

        // 2. Panggil metode 'generateImages' yang baru dan lebih spesifik
        const response = await genAI.models.generateImages({
            // Gunakan model Imagen terbaru yang Anda berikan
            model: 'imagen-4.0-generate-001', 
            prompt: `cinematic, high quality, photorealistic, ${prompt}`, // Tambahkan prefix untuk hasil lebih baik
            config: {
              numberOfImages: 2, // Minta 2 gambar untuk efisiensi
            },
        });

        if (!response.generatedImages || response.generatedImages.length === 0) {
            throw new Error('Model AI tidak menghasilkan gambar. Coba prompt yang berbeda.');
        }

        console.log(`[IMAGEN 4] Berhasil mendapatkan ${response.generatedImages.length} gambar.`);

        // 3. Kirim semua gambar yang berhasil di-generate
        await sock.sendMessage(remoteJid, { text: `✅ Selesai! Berikut adalah hasil imajinasi Anda dengan Imagen 4:` }, { quoted: message });

        for (const generatedImage of response.generatedImages) {
            // Dapatkan data gambar sebagai byte array
            const imgBytes = generatedImage.image.imageBytes;
            
            // Konversi byte array menjadi Buffer
            const imageBuffer = Buffer.from(imgBytes);
            
            await sock.sendMessage(remoteJid, {
                image: imageBuffer,
                caption: `Dibuat dengan Google Imagen 4`
            });
            await new Promise(resolve => setTimeout(resolve, 500)); // Jeda
        }

    } catch (error) {
        console.error("🚫 Imagen 4 Gagal:", error);
...
// --- FITUR GANTI FILE KODE (OWNER ONLY - SANGAT BERBAHAYA) ---
const gantiFileTriggers = ['gantifile', 'ganti file', 'replacefile', 'updatefile'];
const gantiFileTriggerWord = gantiFileTriggers.find(trigger => lowerCaseMessage.startsWith(trigger));

if (gantiFileTriggerWord) {
    // ===================================================================
    // PENGECEKAN KEAMANAN #1: HANYA OWNER YANG BOLEH MENJALANKAN INI
    // ===================================================================
    if (!isOwner(sender)) {
        return { 
            status: true, 
    ...
    // Validasi #1: Pastikan ini adalah sebuah reply
    if (!isQuoted) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: 'Untuk menggunakan fitur ini, Anda harus me-reply pesan yang berisi kode baru.' 
        };
    }
    ...
    if (!filePathToReplace) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: 'Format salah. Contoh: *ganti file lib/ai.js*' 
        };
    }

    // ===================================================================
    // PENGECEKAN KEAMANAN #2: CEGAH SERANGAN PATH TRAVERSAL DAN BATASI HANYA FILE JS
    // ===================================================================
    if (filePathToReplace.includes('..') || !filePathToReplace.endsWith('.js')) {
        return { 
            status: true, 
            message_type: 'text', 
            reply_text: '❌ Path tidak valid atau bukan file .js. Anda hanya bisa mengganti file JavaScript untuk saat ini.' 
        };
    }
    
    // Ambil kode baru dari pesan yang di-reply
    const newCode = message.message.extendedTextMessage.contextInfo.quotedMessage.conversation;
    if (!newCode) {
         return { 
            status: true, 
            message_type: 'text', 
            reply_text: 'Pesan yang di-reply tidak berisi teks kode yang valid.' 
        };
    }

    try {
        // Bangun path absolut yang aman dari root proyek
        // __dirname akan menunjuk ke folder 'lib', jadi kita perlu naik satu level ('..')
        const fullPath = path.resolve(__dirname, '..', filePathToReplace);

        // ===================================================================
        // PENGECEKAN KEAMANAN #3: PASTIKAN PATH TIDAK KELUAR DARI DIREKTORI PROYEK
        // ===================================================================
        const projectRoot = path.resolve(__dirname, '..');
        if (!fullPath.startsWith(projectRoot)) {
             throw new Error('Upaya penulisan file di luar direktori proyek terdeteksi dan diblokir.');
        }

        // Tulis (timpa) file dengan kode baru
        ...
        return {
            status: true,
            message_type: 'text',
            reply_text: `✅ Berhasil! File *${filePathToReplace}* telah diperbarui dengan kode baru.\n\nLauncher akan otomatis me-restart bot untuk menerapkan perubahan.`
        };

    } catch (error) {
        console.error("🚫 Ganti File Gagal:", error);
        return {
            status: true,
            message_type: 'text',
            reply_text: `Gagal memperbarui file.\n\n*Alasan:* ${error.message}`
        };
    }
}
